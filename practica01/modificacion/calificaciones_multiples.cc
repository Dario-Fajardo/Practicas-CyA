...
Lista::Lista(std::string nombre_archivo) {
  ...
  std::string linea;
  std::multimap<std::string, double> lista_sin_ordenar;
  while(getline(fichero, linea)) {
    ...
    alumnos_[id_alumno].insert(stod(nota_alumno));
  }
}
...
void Lista::ImprimirLista() {
  for (const auto& id_actual : identificadores_) {
    std::cout << id_actual << " : ";
    for (const std::pair<std::string, std::multiset<double>>& alumno_actual : alumnos_) {
        if (alumno_actual.first == id_actual) {
          for (const auto& nota_actual : alumno_actual.second) {
            std::cout << nota_actual << " ";
          }
        }
    }
    std::cout << std::endl;
